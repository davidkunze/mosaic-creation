...
path_data = r'Y:\David\vrt_cog\testdaten\duebener_heide_sgb2\daten'
path_out = path_data
...
tile_name = 'st_flugzeug_2023_duebener-heide_dop_sgb2'
...
if not in_srs in [str(out_srs)]:
    ...
    if in_srs in [None,str(9122)]:
        ...
        ogr2ogrString = 'ogr2ogr -s_srs  EPSG:' + str(in_srs) + ' ' + '-t_srs EPSG:' + str(out_srs) + ' ' + inputdata_extent_proj + ' ' + inputdata_extent
    print(ogr2ogrString)
...
if (int(str(height)[:-3]) % 2) == 0:
    height = int(str(height)[:-3]+"000")+2000
else:
    height = int(str(height)[:-3]+"000")+1000

# Convert width to a number with an even number of thousands
if (int(str(height)[:-3]) % 2) == 0:
    width = int(str(width)[:-3]+"000")+2000
else:
    width = int(str(width)[:-3]+"000")+1000
...
for i in range(ulx_int, ulx_int+width, tilesize):
    for j in range(uly_int-height, uly_int, tilesize):
        ...
        print(extentGeometry.Intersect(vectorGeometry))
...
for x in tiles_polygon:
    polygon.append(geopandas.GeoDataFrame(crs='epsg:25832', geometry=[x]))
...
def tiling(input, out_path, extent, count_bands):
    i = 1
    bands = []
    ...
    # overviews of the 8-bit images can be created with the more space-saving JPEG compression, compression method overviewa of 16-bit-images: DEFLATE  
    if dtype == 'Byte':
        gdaltranString = 'gdal_translate -q -of COG -co COMPRESS=DEFLATE -co PREDICTOR=2 -a_srs EPSG:' + str(
        out_srs) + ' ' + bands + ' -co BIGTIFF=YES --config GDAL_TIFF_INTERNAL_MASK YES -co OVERVIEWS=IGNORE_EXISTING -co OVERVIEW_COMPRESS=JPEG -co OVERVIEW_RESAMPLING=bilinear -co OVERVIEW_QUALITY=50 -projwin ' + str(extent[0]) + ', ' + str(extent[1]) + ', ' + str(extent[2]) + ', ' + str(extent[3]) + ' ' + input + ' ' + output
    else:
        gdaltranString = 'gdal_translate -q -of COG -co COMPRESS=DEFLATE -co PREDICTOR=2 -a_srs EPSG:' + str(
        out_srs) + ' ' + bands + ' -co BIGTIFF=YES --config GDAL_TIFF_INTERNAL_MASK YES -co OVERVIEWS=IGNORE_EXISTING -co OVERVIEW_COMPRESS=DEFLATE -co OVERVIEW_RESAMPLING=bilinear -co OVERVIEW_QUALITY=50 -projwin ' + str(extent[0]) + ', ' + str(extent[1]) + ', ' + str(extent[2]) + ', ' + str(extent[3]) + ' ' + input + ' ' + output 
...
if __name__ == '__main__':
    count = mp.cpu_count()
    pool = mp.Pool(count-0)
    args = [(vrt_temp, dir_cog, x, band_count) for x in tiles]
    ...
    # remove temporary vector layers
    ...
    if band_count == 1:
        gdaladdoString = 'gdaladdo -b 1 -r nearest --config GDAL_NUM_THREADS ALL_CPUS --config BIGTIFF IF_SAFER --config OVERVIEW_COMPRESS DEFLATE --config GDAL_TIFF_INTERNAL_MASK YES ' + vrt + ' 16 32 64 128 256 512'
    elif band_count == 2:
        gdaladdoString = 'gdaladdo -b 1 -b 2 -r nearest --config GDAL_NUM_THREADS ALL_CPUS --config BIGTIFF IF_SAFER --config OVERVIEW_COMPRESS DEFLATE --config GDAL_TIFF_INTERNAL_MASK YES ' + vrt + ' 16 32 64 128 256 512'
    elif band_count == 3:
        gdaladdoString = 'gdaladdo -b 1 -b 2 -b 3 -r nearest --config GDAL_NUM_THREADS ALL_CPUS --config BIGTIFF IF_SAFER --config OVERVIEW_COMPRESS DEFLATE --config GDAL_TIFF_INTERNAL_MASK YES ' + vrt + ' 16 32 64 128 256 512'
    elif band_count == 4:
        gdaladdoString = 'gdaladdo -b 1 -b 2 -b 3 -b 4 -r nearest --config GDAL_NUM_THREADS ALL_CPUS --config BIGTIFF IF_SAFER --config OVERVIEW_COMPRESS DEFLATE --config GDAL_TIFF_INTERNAL_MASK YES ' + vrt + ' 16 32 64 128 256 512'
    elif band_count > 4:
        gdaladdoString = 'gdaladdo -b 1 -b 2 -b 3 -r nearest --config GDAL_NUM_THREADS ALL_CPUS --config BIGTIFF IF_SAFER --config OVERVIEW_COMPRESS DEFLATE --config GDAL_TIFF_INTERNAL_MASK YES ' + vrt + ' 16 32 64 128 256 512'
